void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // Uniamo 3 strati di onde sovrapposte
    for(float i = 1.0; i <= 3.0; i++) {
        // Distorciamo lo spazio in modo circolare e fluido
        vec2 p = uv;
        p.x += sin(p.y * 2.0 * i + iTime * 0.5) * 0.2;
        p.y += cos(p.x * 2.0 * i - iTime * 0.5) * 0.2;
        
        // Disegniamo la linea d'onda
        float linea = abs(sin(p.y * 5.0 + iTime * i));
        
        // Effetto bagliore (glow) stile neon
        float spessore = 0.008 / (linea + 0.002);
        
        // Creiamo una palette dinamica (Viola, Magenta, Ciano) che scorre
        vec3 colore = 0.5 + 0.5 * cos(iTime * 0.8 + p.xyx * 3.0 + vec3(0.0, 2.0, 4.0));
        
        coloreFinale += colore * spessore;
    }
    
    // Vignettatura pesante per scurire i bordi e salvare la batteria (OLED)
